...
- **"Today" label** — a Daily Note whose filename matches today's date is shown with a configurable label (default `✘ Today, Mar 4th`), updating automatically at midnight
...
|---|---|
| `2026-03-04` *(today, bare daily note)* | `✘ Today, Mar 4th` |
...
3. Whenever a `.nav-file-title-content` element is painted or updated:
   - If the filename exactly equals today's date and **Show Today label** is on → replaced with the configured label (e.g. `✘ Today, Mar 4th`)
...
| **→ Label format** | `[✘ Today, ]MMM Do` | Moment.js format for the Today label. Wrap literal text in `[...]`. |
